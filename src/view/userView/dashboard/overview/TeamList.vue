<template>
  <BorderLessHeading>
    <sdCards more title="Agency Members">
      <template #more>
        <router-link to="#">
              ...
              <a-list-item>
                <a-list-item-meta>
                  <template #avatar>
                    <a-avatar
                      shape="square"
                      :src="require(`@/static/img/users/${item.avatar}`)"
                    />
                  </template>
                  <template #title>
                    {{ item.title }}
...
import teamList from "@/demoData/teamList.json";

export default defineComponent({
  components: { TeamListStyle, BorderLessHeading },
  setup() {
    const data = ref(teamList);
