      ...
      <div :class="installed ? 'card-short__bottom installed' : 'card-short__bottom'">
        <div class="card-short-actions">
          <div class="content-not-installed content-action">
            <router-link v-if="showLogin" :to="loginRoute">
              <sdButton type="primary" size="sm" shape="circle" raised rounded> Login </sdButton>
            </router-link>

            <router-link v-if="showSignup" :to="signupRoute">
              <sdButton type="light" size="sm" shape="circle" raised rounded> SignUp </sdButton>
            </router-link>
          </div>
        </div>
      </div>
...
export default defineComponent({
  ...
  props: {
    item: VueTypes.object.def({
      ...
      loginRoute: VueTypes.string.isRequired,
      signupRoute: VueTypes.string.def(''),
      showLogin: VueTypes.bool.def(true),
      showSignup: VueTypes.bool.def(false),
    }),
  },
  setup(props) {
    const { item } = toRefs(props);
    const { id, title, installed, content, img, loginRoute, signupRoute, showLogin, showSignup } = item.value;
    return { id, title, installed, content, img, loginRoute, signupRoute, showLogin, showSignup };
  },
});
</script>
